...
// Type guard untuk memastikan user.role valid
const isValidUserRole = (role: string): role is UserRole => {
  return ["admin", "dosen", "mahasiswa"].includes(role);
};
...
const menuConfig: MenuItem[] = [
  ...
  { label: "Kelas", icon: School, path: "/kelas", role: ["admin"] },
    ...
    return (
      ...
      >
        ...
        >
          <div
            data-sidebar="sidebar"
            className="flex h-full w-full flex-col bg-sidebar border-r border-sidebar-border/50 group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow"
          >
            <SidebarHeader>
              <div className="flex items-center gap-3 px-4 py-4">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10 text-primary shadow-sm ring-1 ring-primary/20 transition-all group-hover:scale-105">
                  <School className="h-6 w-6" />
                </div>
                <div className="grid flex-1 text-left leading-tight transition-all duration-200 group-data-[collapsible=icon]:opacity-0 group-data-[collapsible=icon]:w-0">
                  <span className="truncate font-bold text-base tracking-tight">
                    SIAKAD
                  </span>
                  <span className="truncate text-xs text-muted-foreground font-medium">
...
>(({ className, ...props }, ref) => {
  ...
  const filteredMenuItems = menuConfig.filter((item) => {
    ...
    return isValidUserRole(user.role) && item.role.includes(user.role);
  });

  const handleNavigation = (path: string) => {
    const fullPath = `/${user?.role}${path}`;
    navigate(fullPath);
  };
...
// Keep all the existing component definitions below (they remain the same)
